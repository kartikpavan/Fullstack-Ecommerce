...
const CheckoutSummary = () => {
	...
	return (
		<>
			...
			<div className="mt-2">
				<p className="text-sm font-light text-gray-500">Cart Item(s): {totalQuantity} </p>
				<div className="flex items-center justify-between">
					<h1 className="text-xl font-light">Subtotal: </h1>
					<p className="text-primary text-xl font-semibold">{formatPrice(totalAmount)}</p>
				</div>
				{cartItems.map((item) => {
					const { id, name, price, qty } = item;
					return (
						<section className="border-2 border-secondary rounded-md my-2 p-2" key={id}>
